...
parse s = map readInts (lines s)

sumPaths :: (Num a, Ord a) => [[a]] -> [a]
sumPaths [] = [0]
sumPaths (x:[]) = x
sumPaths (x:y:xs) = sumPaths ((zipWith max left right) : xs)
    where left = (zipWith (+) x y) ++ [0]
          right = 0 : (zipWith (+) x ys)
          ys = tail y

main = readFile "fucking-triangle.txt" >>= putStrLn . (show.maximum.sumPaths.parse)
